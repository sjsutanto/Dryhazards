...
FOR i=0, N_ELEMENTS(lon)-1 DO BEGIN
  FOR j=0, N_ELEMENTS(lat)-1 DO BEGIN
    IF LSM[i,j] NE -3.40000e+38 THEN BEGIN
      ...
      dummy2bb = dummy2aa[1:*]
      dummy2bb = [dummy2bb,dummy3aa[N_ELEMENTS(dummy3aa)-1]]
      dummyx = dummy2bb-dummy3aa

      pp = WHERE(dummyx GT 1)
      dummy2cc = [dummy2aa[0],dummy2bb[pp]]
      dummy3cc = [dummy3aa[pp],dummy3aa[N_ELEMENTS(dummy3aa)-1]]
      ;STOP
      ;==================================================
      ; if only single hazards then filter out
        FOR k=0, N_ELEMENTS(dummy2cc)-1 DO BEGIN
          dummy5 = REFORM(Compound_dry[i,j,dummy2cc[k]:dummy3cc[k]])          
          
          dummy6 = dummy5[UNIQ(dummy5, SORT(dummy5))]
          pp = WHERE(dummy6 NE 0, count)
          ...
          ;zz = TOTAL(yyy)
              ...
              IF count GE 1 THEN BEGIN
                ...
                IF (dummy6[pp] EQ 1) OR (dummy6[pp] EQ 2) OR (dummy6[pp] EQ 4) THEN CONTINUE
                ;Cascad_eve[i,j] = Cascad_eve[i,j] + 0
                ;Cascad_dur[i,j] = Cascad_dur[i,j] + 0
                ENDIF ELSE BEGIN
                      
                lengt = dummy3cc[k]-dummy2cc[k]
...
hotsp = 0     ; hotspot single hazard Figure 3a
complot = 0   ; compound hotspots Figure 3b
caseve = 1    ; number of cascading events Figure 4a
...
IF casfreq EQ 1 THEN BEGIN
  ...
  Contour, Freq_4, lon, lat, /Cell_Fill, Levels=levels, C_Colors=IndGen(nlevels)+1, /overplot    ; Data with Rhine dot
  SetDecomposedState, currentState

  cgColorbar, Range=[0,70], NColors=nlevels, Bottom=1,XTicklen=1, FONT=0, $
...
IF casdur EQ 1 THEN BEGIN
  ...
  cgColorbar, Range=[0,90], NColors=nlevels, Bottom=1,XTicklen=1, FONT=0, $
    Position=[0.05, 0.06,0.93, 0.09], Charsize=1.2    ;,/INVERTCOLORS
  Map_Continents,/COUNTRIES, COLOR=0, coasts=1, MLINETHICK=1
...
IF caseve EQ 1 THEN BEGIN
  ...
  cgColorbar, Range=[0,40], NColors=nlevels, Bottom=1,XTicklen=1, FONT=0, $
    Position=[0.05, 0.06,0.93, 0.09], Charsize=1.2    ;,/INVERTCOLORS
  Map_Continents,/COUNTRIES, COLOR=0, coasts=1, MLINETHICK=1
...
IF complot EQ 1 THEN BEGIN
  ...
  cgLoadCT, 25, NColors=nlevels, Bottom=1, /Reverse, /Brewer
  SetDecomposedState, 0, CurrentState=currentState
  Contour, Com_DRHW, lon, lat, /Cell_Fill, Levels=levels, C_Colors=IndGen(nlevels)+1, /overplot    ; Data with Rhine dot
  SetDecomposedState, currentState

  cgColorbar, Range=[0,20], NColors=nlevels, Bottom=1,XTicklen=1, FONT=0, $
    Position=[0.05, 0.06,0.93, 0.09], Charsize=1.2    ;,/INVERTCOLORS
  Map_Continents,/COUNTRIES, COLOR=0, coasts=1, MLINETHICK=1
